...
def get_symbols(conn):
    with conn.cursor() as cur:
        cur.execute(
            f"SELECT DISTINCT bse_symbol FROM {MASTER} "
            f"WHERE bse_symbol IS NOT NULL AND bse_symbol <> '' "
            f"AND nse_symbol IS NULL"
        )
...
def main():
    ...
    print(f"Total BSE only symbols : {len(symbols)}")
